...
public class PlaylistHelperService : IPlaylistHelper
{
    ...
    )
    {
        ...
        foreach (var category in categories)
        {
            ...
            {
                artistPlaylistsId[category] = await GetOrCreatePlaylistByNameAsync(
                    spotifyClient,
                    userId,
                    $"{artistName} {category}"
                );
    ...
    )
    {
        ...
        return await GetOrCreatePlaylistByNameAsync(spotifyClient, userId, playlistName);
    }

    private static async Task<string> GetOrCreatePlaylistByNameAsync(
        SpotifyClient spotifyClient,
        string userId,
        string playlistName
    )
    {
        ...
        var existingPlaylist = userPlaylists.FirstOrDefault(p => p.Name == playlistName);
